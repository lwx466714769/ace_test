...
export default class Painter {
  ...
  paint(callback) {
    ...
    this._background();
    for (let view of this.data.views) {
  ...
  _fillAbsText(view) {
    ...
    for (let i = 0; i < lines; ++i) {
      alreadyCount = preLineLength +'' === 'NaN' ? 0 : preLineLength;
      let text = alreadyCount != 0 ? view.text.substr(start, alreadyCount) : '';
      ...
      if (view.css.textDecoration) {
        ...
        if (/\bline-through\b/.test(view.css.textDecoration)) {
          this.ctx.moveTo(x, y - fontSize / 5);
          this.ctx.lineTo(x + measuredWith, y - fontSize / 5);
        }
        this.ctx.closePath();
