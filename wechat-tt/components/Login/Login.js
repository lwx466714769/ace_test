...
Component({
  ...
  methods: {
    ...
    verifyPhone(cb) {
      ...
      let data = { code: code + '' }
      let success = body => {
        // this.bindPhoneNumber(body.secureToken, phone);
        this.hide()
        !!cb && cb({ phone, code })
      }
      let fail = body => {
    ...
    // 绑定手机号 
    bindPhoneNumber(secureToken, phone){

      let data = {
        secureToken,
        phone
      };
      let url = `${urls._bindPhone}/${phone}`;
      let success = body => {
        console.log('phone--> result',body);
      };
      let fail = error => {
        console.log('phone-->error',error);
      };

      return fetch({ url, method: 'POST', data, success, fail });
