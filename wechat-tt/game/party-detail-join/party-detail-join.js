...
Page({
  ...
  getPartyDetail(){
    ...
    fetch({
      ...
      success:body=>{
        ...
        let startTime = body.startTime.split(' ');
        let endTime = body.endTime.split(' ');
        if(startTime[0] == endTime[0]){
          body.sameTime = body.startTime + ' - ' + endTime[1];
        }
   ...
   */
  onShareAppMessage() {
    const { partyId, party, shareImg } = this.data;
    let imageUrl = shareImg || '/images/banner.png';
    let title = party.name;
    let path = `/game/party-detail-join/party-detail-join?partyId=${partyId}&isShare=true`;
  ...
  shareDrawClub() {
    ...
    let background = party.background || '/images/banner.png';
