...
function formatMsgTime(timestamp) {
  ...
  if (milliseconds <= 1000 * 60 * 1) {
    ...
    timestampStr = month + '-' + day + ' ' + hour + ':' + fix(minute);
  } else {
    timestampStr = year + '-' + month + '-' + day + ' ' + hour + ':' + fix(minute);
  }
  return timestampStr;
};
...
// 支付,ping++
function toPayPing(orderId, productType) {
  return new Promise((resolve,reject)=>{
...
//去支付多拉宝
function toPayDLB(orderId,productType){
  return new Promise((resolve,reject)=>{
    let url = `${urls.order}/charge`;
    let data = {
      orderId,
    ...
    let success = body =>{
      let bank = JSON.parse(body)
      wx.requestPayment({
        timeStamp: bank.TIMESTAMP,
        nonceStr: bank.NONCESTR,
        package: bank.PACKAGE,
        signType: bank.SIGNTYPE,
        paySign: bank.PAYSIGN,
        success(res) {
          console.log(res);
          wx.showToast({
            title: '支付成功',
    ...
    fetch({url,method:'POST',data,success,fail,loading:'加载中...'})
  })
...
// 高级会员卡信息
function getSeniorApplyInfo() {
  const clubId = constv.cid;
  return new Promise((resolve,reject)=>{
    fetch({
      url: `${urls.payMember}/searchPayMember?clubId=${clubId}`,
      loading: false,
      success: body => {
        if (body.length>0) {
...
// 查询是否是高级会员
function getSeniorMember(){
  const clubId = constv.cid;
  const { userId } = wx.getStorageSync('user');
  return new Promise((resolve,reject)=>{
    fetch({
      url: `${urls.payMember}/user?clubId=${clubId}&userId=${userId}`,
      loading: false,
      success: body => {        
        if (body.length>0) {
            if(body[body.length-1].invalid == 0){
              resolve('member');
            }else{
              resolve('noMember');
            }
...
// 照片下载原图
function tapDownloadImg(downloadUrl){
  wx.getSetting({
    success:res =>{
...
// 拒绝授权提示
function authPhoto(downloadUrl){
  wx.showModal({
    content:"保存图片需要您的授权",
    success:res=>{
      if(res.confirm){
...
// 下载图片
function downloadImg(downloadUrl){
  wx.showLoading({
    title: '下载中...',
  })
  wx.getImageInfo({
